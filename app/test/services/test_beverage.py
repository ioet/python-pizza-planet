...
def test_update_beverage_service(client, create_beverage, beverage_uri):
    ...
    update_data = {**current_beverage, 'type': get_random_string(), 'price': get_random_price(1, 5)}
...
def test_get_beverage_service(client, create_beverages, beverage_uri):
    ...
    pytest.assume(response.status.startswith('200'))
    returned_beverage = {beverage['_id']: beverage for beverage in response.json}
    for beverage in create_beverages:
        pytest.assume(beverage['_id'] in returned_beverage) 