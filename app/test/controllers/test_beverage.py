...
def test_get_by_id(app, beverage:dict):
    ...
    for param, value in created_beverage.items():
        pytest.assume(param in beverage_from_db)
        pytest.assume(value == beverage_from_db[param])
...
def test_get_all(app, beverages: list):
    ...
    for created_beverage in created_beverages:
        ...
        for param, value in created_beverage.items():
            pytest.assume(param in searchable_beverages[current_id])
            pytest.assume(value == searchable_beverages[current_id][param])
