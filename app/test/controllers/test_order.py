...
def __create_beverages_sizes_and_ingredients(
    ingredients: list, sizes: list, beverages: list
):
    created_ingredients = __create_items(
        items=ingredients, controller=IngredientController
    )
...
def test_create(app, ingredients, beverages, size, client_data):
    (
        ...
        created_ingredients,
        created_beverages,
    ) = __create_beverages_sizes_and_ingredients(
        ingredients=ingredients, sizes=[size], beverages=beverages
    )
    order = __order(
...
def test_get_all(app, ingredients, beverages, sizes, client_data):
    (
        ...
        ingredients=ingredients, sizes=sizes, beverages=beverages
    )
    created_orders = []
    for _ in range(5):
        order = __order(
            ingredients=shuffle_list(created_ingredients)[:3],
            beverages=shuffle_list(created_beverages)[:3],
            size=get_random_choice(created_sizes),
            client_data=client_data,
        )
        created_order, _ = OrderController.create(order=order)
